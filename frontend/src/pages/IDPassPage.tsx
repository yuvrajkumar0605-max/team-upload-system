...
import { Search, IdCard, Users, Link as LinkIcon, Loader2, AlertCircle } from 'lucide-react';
import { Alert, AlertDescription, AlertTitle } from '@/components/ui/alert';
import type { TeamRegistration } from '../backend';

export default function IDPassPage() {
  const [searchTerm, setSearchTerm] = useState('');
  const [searchResult, setSearchResult] = useState<TeamRegistration | null>(null);
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <div className="max-w-4xl mx-auto">
        ...
        {searchMutation.isError && (
          <Alert className="border-yellow-500/50 bg-yellow-500/10">
            ...
            <AlertDescription className="text-muted-foreground">
              No teams match your search. Please check your team name or phone number and try again.
            </AlertDescription>
          </Alert>
        )}
          ...
          <>
            ...
            {!searchResult.idPassAssigned && (
              <Alert className="border-amber-500/50 bg-amber-500/10 mb-6">
                ...
                <AlertTitle className="text-amber-500">ID Pass Not Yet Assigned</AlertTitle>
                <AlertDescription className="text-muted-foreground">
                  Your ID Pass is not yet assigned. Please check back later or contact the tournament organizers for more information.
                </AlertDescription>
              </Alert>
            )}
            ...
            <Card className="border-primary/50 bg-card/50 backdrop-blur-sm glow-primary">
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex items-center gap-6 pb-6 border-b border-border">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-primary">{searchResult.teamName}</h3>
                    <p className="text-muted-foreground">Captain: {searchResult.captain.name}</p>
                  </div>
                </div>
                ...
                {searchResult.idPassAssigned ? (
                  <>
                    ...
                    {!searchResult.groupLink && (
                      <Alert className="border-blue-500/50 bg-blue-500/10">
                        <AlertDescription className="text-muted-foreground">
                          Group link will be available soon. Please check back later.
                        </AlertDescription>
                      </Alert>
                    )}
                  </>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-lg font-medium text-muted-foreground">
                      Your team has been registered successfully, but your ID Pass details are not yet available.
                    </p>
        ...
        {!searchResult && !searchMutation.isError && !searchMutation.isPending && (
