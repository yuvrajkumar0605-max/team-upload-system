...
import { useGetAllRegistrations, useDeleteRegistration, useGetAnnouncementsCount, useGetResultsCount, useGetRegistrationStatus, useCloseRegistration, useSetMaxTeamRegistrations, useStoreReport, useGetRegistrationSummary, useSearchTeamsByName, useGetAllTeamLogos } from '../hooks/useQueries';
...
import { XCircle, Trash2, Search, Users, Megaphone, Trophy, DoorOpen, DoorClosed, Share2, Infinity, Save, Eye, Download, FileText, AlertTriangle, IdCard, Calendar, CheckCircle, FileSpreadsheet, X, Image as ImageIcon } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const [searchResults, setSearchResults] = useState<TeamRegistration[] | null>(null);
  const [isSearching, setIsSearching] = useState(false);
  const [confirmAction, setConfirmAction] = useState<{ type: 'delete'; registration: TeamRegistration } | null>(null);
  const [isExporting, setIsExporting] = useState(false);
  
  // Registration limit state
  const [limitInput, setLimitInput] = useState('');
  const [isUnlimited, setIsUnlimited] = useState(summary?.maxTeams === undefined);
  ...
  const displayedRegistrations = useMemo(() => {
    ...
    // Sort by team name
    return [...teamsToDisplay].sort((a, b) => a.teamName.localeCompare(b.teamName));
  ...
  const stats = useMemo(() => {
    ...
    const limit = summary?.maxTeams !== undefined ? Number(summary.maxTeams) : null;
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(`Team "${confirmAction.registration.teamName}" deleted successfully`);
    } catch (error: any) {
  ...
  const handleToggleRegistration = async (checked: boolean) => {
    try {
      ...
      toast.error(`Failed to ${checked ? 'open' : 'close'} registration: ${error.message}`);
    }
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    try {
      ...
      const externalBlob = ExternalBlob.fromBytes(new Uint8Array(await csvBlob.arrayBuffer()));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-primary mb-2 glow-text">Battle of Supremacy - Admin Dashboard</h1>
          <p className="text-muted-foreground">Manage tournament registrations, announcements, and results</p>
        </div>
        <Button
      ...
      <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            {isRegistrationOpen ? <DoorOpen className="w-5 h-5 text-green-500" /> : <DoorClosed className="w-5 h-5 text-red-500" />}
            Registration Control
          </CardTitle>
          <CardDescription>Enable or disable team registrations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex items-center justify-between p-4 border border-primary/20 rounded-lg bg-primary/5">
            <div className="flex items-center gap-3">
              <Label htmlFor="registration-toggle" className="text-base font-medium cursor-pointer">
                Registration Status
              </Label>
      ...
      <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>Set maximum number of team registrations</CardDescription>
        </CardHeader>
          ...
          <div className="flex items-center justify-between p-4 border border-primary/20 rounded-lg bg-primary/5">
            <div>
              ...
              <p className="text-2xl font-bold text-primary flex items-center gap-2">
                {stats.limit !== null ? (
                  <span>{stats.total} / {stats.limit} teams registered</span>
                ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {stats.limit !== null ? `${stats.limit - stats.total} spots remaining` : 'Unlimited spots'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-purple-500/20 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-500">{stats.announcements}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-amber-500/20 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-500">{stats.results}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-blue-500/20 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-500">{stats.logos}</div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="teams" className="space-y-6">
        <TabsList className="grid w-full max-w-6xl grid-cols-5 lg:grid-cols-11">
        ...
        <TabsContent value="teams" className="space-y-6">
          ...
          <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
            <CardHeader>
              ...
              <CardDescription>Review and manage team registrations (all teams are auto-approved)</CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
                  {isSearching ? (
                    ...
                    <>
                      ...
                      <span>
                        Found {displayedRegistrations.length} team{displayedRegistrations.length !== 1 ? 's' : ''} matching "{searchTerm}"
                      </span>
                    </>
                  ) : null}
                </div>
              ...
              {displayedRegistrations.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p>{searchTerm ? 'No teams found matching your search' : 'No registrations found'}</p>
                  {searchTerm && (
                    <Button
                      variant="outline"
                      onClick={handleClearSearch}
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {displayedRegistrations.map((registration) => (
                        <TableRow key={registration.id} className="hover:bg-primary/5">
                          ...
                          <TableCell>
                            <div className="text-sm text-muted-foreground">{registration.captain.phone}</div>
                          </TableCell>
          ...
          <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
            ...
            <CardContent>
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-lg font-medium mb-2">Schedule Management Coming Soon</p>
                <p className="text-sm">
                  Schedule upload and management features will be available in the next update.
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      <AlertDialog open={confirmAction?.type === 'delete'} onOpenChange={(open) => !open && setConfirmAction(null)}>
        <AlertDialogContent className="border-destructive/20 bg-card/95 backdrop-blur-sm">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the team <span className="font-semibold text-foreground">"{confirmAction?.registration.teamName}"</span>?
              <br /><br />
              <span className="text-destructive font-medium">⚠️ This action cannot be undone.</span> All team data will be permanently removed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
