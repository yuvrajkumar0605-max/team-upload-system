...
import { useGetAllTeamsWithIdPassStatus, useGetAllGroupLinks, useAddGroupLink, useUpdateIdPassStatus, useBulkUpdateIdPassStatus } from '../hooks/useQueries';
...
import { IdCard, Users, Link as LinkIcon, Save, Loader2, Search, CheckCircle, XCircle, Filter } from 'lucide-react';
...
export default function IDPassManagement() {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'assigned' | 'not-assigned'>('all');
  ...
  useMemo(() => {
    if (groupLinks.length > 0) {
      ...
      groupLinks.forEach(([groupNum, link]) => {
  ...
  const handleSaveLink = async (groupNumber: number) => {
    ...
    try {
      await addGroupLink.mutateAsync({ groupNumber: BigInt(groupNumber), link });
  ...
  const handleBulkUnassign = async () => {
    try {
      ...
      toast.error(`Failed to unassign ID Pass from all teams: ${error.message}`);
    }
  };
  ...
  const totalTeams = teams.length;
  const teamsPerGroup = Math.ceil(totalTeams / 18);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="border-green-500/20 bg-card/50 backdrop-blur-sm">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">ID Pass Assigned</CardTitle>
            <CheckCircle className="h-4 w-4 text-green-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-green-500">{stats.assignedTeams}</div>
          </CardContent>
        </Card>
        ...
        <Card className="border-red-500/20 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-500">{stats.notAssignedTeams}</div>
          </CardContent>
        </Card>
      ...
      <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>Manage ID Pass assignment for individual teams</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Select value={filterStatus} onValueChange={(value: any) => setFilterStatus(value)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="not-assigned">Not Assigned Only</SelectItem>
                </SelectContent>
              </Select>
            </div>
                      ...
                      <TableRow key={team.id} className="hover:bg-primary/5">
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            onClick={() => handleToggleIdPass(team.teamId, team.idPassAssigned)}
      ...
      <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>
            Enter 18 unique group links (one per line) to assign to all groups at once
          </CardDescription>
        </CardHeader>
      ...
      <Card className="border-primary/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>Edit links for each group individually</CardDescription>
        </CardHeader>
        <CardContent>
          <Accordion type="single" collapsible className="w-full">
            {Array.from({ length: 18 }, (_, i) => i + 1).map((groupNum) => {
              const groupTeams = teamsByGroup[groupNum];
              const assignedCount = groupTeams.filter(t => t.idPassAssigned).length;
                  ...
                  <AccordionTrigger className="hover:no-underline">
                    <div className="flex items-center justify-between w-full pr-4">
                      ...
                      <div className="flex items-center gap-3 text-sm text-muted-foreground">
                        ...
                        <span className="text-green-500">{assignedCount} assigned</span>
                      </div>
                    </div>
                  </AccordionTrigger>
                  <AccordionContent>
                    <div className="space-y-4 pt-4">
                      ...
                      {groupTeams.length > 0 && (
                        <div className="text-sm text-muted-foreground">
                          <p className="font-medium mb-2">Teams in this group:</p>
