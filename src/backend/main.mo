...
actor {
  ...
  // ---- BANNER MANAGEMENT ----
  public shared ({ caller }) func setBanner(image : Storage.ExternalBlob) : async { #ok : Text; #err : Text } {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      return #err("Unauthorized: Only admins can set the banner");
    };
    let banner : Banner = {
      id = "banner";
      image;
  ...
  public shared ({ caller }) func deleteBanner() : async { #ok : Text; #err : Text } {
    if (not (AccessControl.hasPermission(accessControlState, caller, #admin))) {
      return #err("Unauthorized: Only admins can delete the banner");
    };
    optBanner := null;
    #ok("Banner deleted successfully");
  };

  public query func getBanner() : async ?Banner {
    optBanner;
  };
