...
import {
  useDeleteDuplicateEntry,
  useGetAllRegistrations,
  useGetDuplicateRegistrations,
} from "../hooks/useQueries";
import type { DuplicateEntry, TeamRegistration } from "../types";
import { DuplicateReason } from "../types";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export default function DuplicateRegistrationsManagement() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<DuplicateEntry | null>(
    null,
  );
  const [viewDetails, setViewDetails] = useState<{
    original: TeamRegistration;
    duplicate: TeamRegistration;
  ...
  const filteredDuplicates = useMemo(() => {
    return duplicates.filter((dup) => {
      ...
      const matchesSearch =
        ...
        original.captain.name
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
        duplicate.captain.name
          .toLowerCase()
          .includes(searchTerm.toLowerCase()) ||
  ...
  const confirmDeleteAction = async () => {
    ...
    const reason =
      confirmDelete.reason === DuplicateReason.teamName ? "team-name" : "phone";
      ...
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            Loading duplicate registrations...
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <Card className="border-amber-500/20 bg-card/50 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription>
            Review and manage duplicate team registrations detected by the
            system
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredDuplicates.map((duplicate, index) => {
                    ...
                    return (
                      <TableRow
                        key={`${duplicate.originalId}-${duplicate.duplicateId}-${index}`}
                        ...
                        <TableCell>
                          <div className="space-y-2">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-medium">
                                  {duplicateReg.teamName}
                                </div>
                                <div className="text-sm text-muted-foreground">
                                  {duplicateReg.captain.name}
                                </div>
                              </div>
                            </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="text-destructive font-medium">
                ⚠️ This will permanently delete the duplicate team registration.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-lg">
                        {viewDetails.original.teamName}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Team ID: {viewDetails.original.id}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {viewDetails.original.captain.name}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {viewDetails.original.captain.phone}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-2">
                      Team Members ({viewDetails.original.members.length})
                    </p>
                    <div className="space-y-1">
                      {viewDetails.original.members.map((member) => (
                        <div
                          key={member.name}
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-lg">
                        {viewDetails.duplicate.teamName}
                      </p>
                      <p className="text-sm text-muted-foreground">
                        Team ID: {viewDetails.duplicate.id}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">
                      {viewDetails.duplicate.captain.name}
                    </p>
                    <p className="text-sm text-muted-foreground">
                      {viewDetails.duplicate.captain.phone}
                    </p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground mb-2">
                      Team Members ({viewDetails.duplicate.members.length})
                    </p>
                    <div className="space-y-1">
                      {viewDetails.duplicate.members.map((member) => (
                        <div
                          key={member.name}
                          className="text-sm p-2 bg-primary/5 rounded border border-primary/10"
                        >
